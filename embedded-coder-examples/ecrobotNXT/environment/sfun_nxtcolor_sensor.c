/*
 * sfun_nxtcolor_sensor.c: simulation stub for NXT Color Sensor
 *
 *   Copyright 2010 The MathWorks, Inc.
 */


#define S_FUNCTION_NAME  sfun_nxtcolor_sensor
...
#define SENSOR_MODE_PRM(S)         (ssGetSFcnParam(S,4))
...
#define SENSOR_MODE                ((uint8_T) mxGetPr(SENSOR_MODE_PRM(S))[0])

/* NXT Color Sensor modes */
#define LIGHTSENSOR_NONE           (1)
#define LIGHTSENSOR_RED            (2)
#define LIGHTSENSOR_GREEN          (3)
#define LIGHTSENSOR_BLUE           (4)
#define LIGHTSENSOR_FULL           (5)
#define COLORSENSOR_ID             (6)
#define COLORSENSOR_RGB            (7)

#define INTERFACE_BLOCK  0
#define GET_BLOCK        1

#define NUM_OF_BD_MODES  2  /* Master/Slave */
#define SIZE_DATA_PACKET 3

/*==================================================*
 * Macros to access the S-function parameter values *
 *==================================================*/
int32_T receiveBuf[NUM_OF_BD_MODES][4][SIZE_DATA_PACKET];
uint8_T port_config[NUM_OF_BD_MODES][2];
uint8_T sensor_mode_config[NUM_OF_BD_MODES][2];
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (BLOCK_MODE == INTERFACE_BLOCK)
    {
        ...
        if (SENSOR_MODE == COLORSENSOR_RGB)
        {
        ...
        sensor_mode_config[BD_MODE][0] = SENSOR_MODE;
    }
...
//        ssSetOutputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
        ...
        sensor_mode_config[BD_MODE][1] = SENSOR_MODE;
    }
    ...
    if (port_config[BD_MODE][0] != 0 && port_config[BD_MODE][1] != 0)
    {
        ...
        {
            ssSetErrorStatus(S, "Port configuration mismatch on NXT Color Sensor Interface/Read blocks or multiple uses.");
        }
    }

    if (sensor_mode_config[BD_MODE][0] != 0 && sensor_mode_config[BD_MODE][1] != 0)
    {
        if (sensor_mode_config[BD_MODE][0] != sensor_mode_config[BD_MODE][1])
        {
            ssSetErrorStatus(S, "Sensor mode configuration mismatch on NXT Color Sensor Interface/Read blocks or multiple uses.");
        }
    }
    ...
    memset(sensor_mode_config,0,sizeof(sensor_mode_config));
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  if (num_inports == 1 && num_outports == 0) 
  {
      ...
      {
        u = (uint16_T *)ssGetInputPortSignal(S, 0);
        receiveBuf[BD_MODE][PORT][0] = *u;
      }
  ...
  else if (num_inports == 0 && num_outports == 1)
  {
      if (SENSOR_MODE == COLORSENSOR_RGB)
      {
        ...
        for (i = 0; i < SIZE_DATA_PACKET; i++)
        {
            y[i] = (int16_T)receiveBuf[BD_MODE][PORT][i];
        }
      }
      else
      {
        y = (uint16_T *)ssGetOutputPortSignal(S, 0);
        *y = (uint16_T)receiveBuf[BD_MODE][PORT][0];
      }
  }
}
...
static void mdlRTW(SimStruct *S)
{
  ...
  const uint8_T sensor_mode = SENSOR_MODE;
  ...
  if (!ssWriteRTWParamSettings(S, 5,
       ...
       SSWRITE_VALUE_DTYPE_NUM, "sensor_mode", &sensor_mode, DTINFO(SS_UINT8,COMPLEX_NO)))
  {
    ssSetErrorStatus(S, "NXT Color Sensor:ParamError");
