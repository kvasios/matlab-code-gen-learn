...
rtmGetErrorStatus(untitled_M) == (NULL))
    ...
    /* Get model outputs here */
      ...
      step_index++;
      {
        time_T FinalTime = rtmGetFinalTime(untitled_M);
        if((FinalTime >= 0.0) && ((step_index * 0.5) > FinalTime)) {
          rtmSetStopRequested(untitled_M, TRUE);
        }
      }
      #endif
      {
        clock_ns += uint64_t(0.5 * 1000000000);
  ...
  int_T main(void)
  {
    ...
    rtmSetStopRequested(untitled_M, FALSE);
    #endif
