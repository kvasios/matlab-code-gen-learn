...
#define S_FUNCTION_NAME		sfun_comedi_ai
...
extern comedi_range *lnx_comedi_get_range(void *dev, unsigned int subdev, unsigned int chan, unsigned int range);
...
static void mdlStart(SimStruct *S)
{
	...
	lnx_range = lnx_comedi_get_range(dev, subdev, channel, range);
	if (lnx_range->min >= lnx_range->max) {
	...
	range_min = (double)(lnx_range->min);
	range_max = (double)(lnx_range->max);
	printf("AI Channel %d - Range : %1.2f [V] - %1.2f [V] (maxdata 0x%lx)\n",
		channel, range_min, range_max,
		(unsigned long)comedi_get_maxdata(dev, subdev, channel));
