 ...
 *
 * File: rtiostream_tcpip.c     $Revision: 19727 $
 *
...
static SOCKET clientStreamOpen(
    char * hostName, 
    ...
    size_t        *sizeRecvd)
{
    ...
    *sizeRecvd = recv(sock, dst, (int)size, 0U);
    if ((int)*sizeRecvd == SOCK_ERR) {
        perror("recv");
        retVal = RTIOSTREAM_ERROR;
    }
    ...
    size_t *sizeSent)
{
    ...
    *sizeSent = send(sock, src, size>(size_t)INT_MAX?(size_t)INT_MAX:size, 0);
    if ((int)*sizeSent == SOCK_ERR) {
        perror("send");
        retVal = RTIOSTREAM_ERROR;
    }
    ...
    size_t * sizeRecvd)
{
    ...
    *sizeRecvd = 0;
    ...
    if (ServerData.sock != INVALID_SOCKET) {
        retVal = socketDataPending(ServerData.sock, &pending, RecvTimeoutSecs);
...
static SOCKET serverOpenConnection(void)
{
    ...
    error = socketDataPending(lFd, &pending, RecvTimeoutSecs);
    ...
    unsigned int * recvTimeout)
{
    ...
    while(count < argc) {
        ...
        if (option != NULL) {
            ...
            } else if ((strcmp(option, "-l") == 0) && (count != argc)) {
                char       tmpstr[2];
                int itemsConverted;
...
int rtIOStreamOpen(int argc, void * argv[])
{
    ...
    processArgs(argc, argv, &xHostName, &xPortNum, &isClient, &RecvTimeoutSecs);
    ...
    size_t * sizeRecvd)
{
    ...
    if (streamID == SERVER_STREAM_ID)
        ...
        retVal = socketDataPending(cSock,&pending,RecvTimeoutSecs);
...
int rtIOStreamClose(int streamID)
{
    ...
    if (streamID == SERVER_STREAM_ID) {
        ...
        if (ServerData.sock > 0) {
            do {
                retVal = rtIOStreamRecv(streamID, tmpBuf, TMP_BUF_SIZ, &numRecvd);
            } while ((retVal == RTIOSTREAM_NO_ERROR) && (numRecvd > 0));
